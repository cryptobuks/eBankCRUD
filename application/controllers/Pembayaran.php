...
class pembayaran extends CI_Controller{
    ...
    public function getAllUserData(){
        ...
        $data['saved']      = $this->pembayaranmodel->showSavedAccountNumber($data['detailID']);
        ...
        $data['form']       = $this->load->view('include/main/pembayaran/form', array('detailID' => $data['detailID'], 'save' => $data['saved']), TRUE);
        $data['footer']     = $this->load->view('include/main/footer', NULL, TRUE);
        $data['form_js']    = $this->load->view('include/main/js_formFunction_pembayaran', NULL, TRUE);
    ...
    public function generateOTP($length = 5) {
		...
		/*
		go to pembayaran/test to see otp code
        //get user phone number
        //$phoneDestination = $this->session->userdata('phone');

        //begin of call mesabot library To send SMS
        $this->load->library('Mesabot');
        try {
            $data = [
                'destination' => '083895974628',
                'text' => $randomString
            ];
    ...
    public function saveAccountNumber(){
        ...
        $this->load->model('pembayaranmodel');
        $this->pembayaranmodel->saveAccountNumberToDb($uid, $entered, $type);
    }
    ...
    public function index(){
        ...
        if($logged == true){
            ...
            $this->load->view('main/pembayaran.php', $data);
        }
    }
}